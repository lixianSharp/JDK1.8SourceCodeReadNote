...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * (See below forTreeNode subclass, and in LinkedHashMap for its Entry subclass.)
     *
     * 基本的hash表节点，
     ...
     * 构造一个空的HashMap,使用专门的初始化容量和负载因子。
     * @param  initialCapacity the initial capacity  初始化容量
     * @param  loadFactor      the load factor  负载因子
     * @throws IllegalArgumentException 如果初始化容量或负载因子为负数，则抛出IllegalArgumentException异常。
     */
    public HashMap(int initialCapacity, float loadFactor) {
        ...
        if (loadFactor <= 0 || Float.isNaN(loadFactor))
            throw new IllegalArgumentException("Illegal load factor: " +
                                               loadFactor);
        this.loadFactor = loadFactor;//给负载因子赋值
        this.threshold = tableSizeFor(initialCapacity);////要调整大小的下一个大小值(容量*负载因子)。
    }
     ...
     *
     * @return the number of key-value mappings in this map 返回此映射中键值映射的数目。
     */
    public int size() {
        return size;
    }
     ...
     *  判断是否为空
     * @return <tt>true</tt> if this map contains no key-value mappings 当次map中不含键值对的时候，返回true.
     */
     ...
     * 替换给定哈希表的在索引处中的所有链接节点，除非表太小，否则在这种情况下将调整大小。
     */
     ...
     * 进入树箱。LinkedHashMap.Entry的扩展(依次扩展节点)，因此可以用作常规节点或链接节点的扩展。
     * 这是一个红黑树
     */
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        TreeNode<K,V> parent;  // red-black tree links 红黑树链表的父节点
        TreeNode<K,V> left; //左孩子
        TreeNode<K,V> right; //右孩子
        TreeNode<K,V> prev;    // needed to unlink next upon deletion  前置节点
        boolean red;
